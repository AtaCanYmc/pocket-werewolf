...
export default function DayPhase() {
  ...
  return (
    <div className="max-w-5xl w-full mx-auto px-3 sm:px-4 py-2 sm:py-4 space-y-4 sm:space-y-6 animate-fade-in">
      {/* Day Header */}
      <div className="bg-surface border border-surface-border rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-3 sm:gap-4 text-center sm:text-left">
          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-2xl sm:text-3xl shadow-lg flex-shrink-0">
            ☀️
          </div>
          <div>
            ...
            <h2 className="font-gothic text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-100">{t('day.title')}</h2>
      ...
      {/* Host Timer Controls */}
      {isHost && (
        <div className="flex items-center justify-between p-3 rounded-xl bg-surface-light border border-slate-200 dark:border-slate-800 text-xs">
        ...
        <div className="lg:col-span-6 space-y-4">
          ...
          <div className="bg-surface border border-surface-border rounded-2xl p-4 sm:p-5 shadow-xl">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              ...
              <span className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">{t('day.aliveSubtitle')}</span>
            </div>
              ...
              {alivePlayers.map((p) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 sm:p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all active:scale-[0.99] ${
                      isSuspect
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 truncate">
                          ...
                          {isMe && (
                            <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-medium flex-shrink-0">
                              ({t('lobby.you')})
                            </span>
                          )}
                        </div>
                    ...
                    {isSuspect && (
                      <span className="text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-950 px-2 py-0.5 rounded border border-red-300 dark:border-red-800 flex-shrink-0 ml-1">
                        {t('day.suspectBadge')}
                      </span>
                    )}
                  </div>
      ...
      {isHost ? (
        <div className="bg-surface border border-surface-border rounded-2xl p-4 sm:p-5 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          ...
          <button
            ...
            className="w-full sm:w-auto py-3 sm:py-3.5 px-6 sm:px-8 rounded-xl bg-red-600 hover:bg-red-500 text-white font-gothic font-bold text-xs sm:text-sm tracking-wider uppercase transition-all shadow-blood-glow flex items-center justify-center gap-2 active:scale-[0.99] flex-shrink-0"
          >
        ...
        <div className="p-3 sm:p-3.5 rounded-xl bg-surface-light border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 text-center animate-pulse">
          {t('day.waitingHost')}
        </div>
      )}
    </div>
